  ...
  <div class="divide-y divide-gray-300 rounded-3xl bg-gray-100 shadow-xl pb-4">
    ...
    >
      ...
      <!-- Simple SVG download button. Should be hidden when there are no seats or no parties 
         ...
         -->
      <a
        v-if="list.length != 0 && diagram.totalSeats > 0"
        :href="downloadURL"
        :download="filename"
        class="
          flex-shrink
    ...
    <div v-if="list.length != 0" v-html="diagram.svg"></div>
...
let diagram = computed(() => {
  return new ArchDiagram(props.list, props.options);
});
...
let filename = computed(() => {
  return encodeURIComponent(props.options.diagramTitle) + "_" + Date.now() + ".svg";
});
